...
`CPN.toLPE_cond` says the condition Definition 14 builds evaluates to `CPN.Enabled d b`, so
each of these is that theorem followed by the matching enabledness lemma of
`Examples/CounterNet.lean`. -/

/-- `c_{t₁}(d, h) = {1`h} ⊆ s_p₁`. The guard of `t₁` is `True`, and `pre(t₁) = {p₁}`. -/
theorem cond_t₁ (d : net.Marking) (b : net.TransBinding .t₁) :
    net.toLPE.cond .t₁ d b ↔ Bag.single 1 (valueOf b) ⊆ d .p₁ := by
  rw [CPN.toLPE_cond]; exact enabled_t₁_iff d b

/-- `c_{t₂}(d, h) = {1`h} ⊆ s_p₂ ∧ h ≤ 3`. -/
theorem cond_t₂ (d : net.Marking) (b : net.TransBinding .t₂) :
    net.toLPE.cond .t₂ d b ↔ Bag.single 1 (valueOf b) ⊆ d .p₂ ∧ valueOf b ≤ 3 := by
  rw [CPN.toLPE_cond]; exact enabled_t₂_iff d b

/-- `c_{t₃}(d, h) = {1`h} ⊆ s_p₂ ∧ h > 3`, over `s_p₂` and not `s_p₃`. -/
theorem cond_t₃ (d : net.Marking) (b : net.TransBinding .t₃) :
    net.toLPE.cond .t₃ d b ↔ Bag.single 1 (valueOf b) ⊆ d .p₂ ∧ 3 < valueOf b := by
  rw [CPN.toLPE_cond]; exact enabled_t₃_iff d b

/-! ### The next state of the first summand

`g_{t₁}(d, h) = (s_p₁ \ {1`h}, s_p₂ ∪ {1`(h+1)}, s_p₃)`. Each component is one place of the
marking `CPN.fire d b` of `Examples/CounterNet.lean`, which `CPN.toLPE_next` says the terms
Definition 14 builds evaluate to. -/

theorem next_t₁_p₁ (d : net.Marking) (b : net.TransBinding .t₁) :
    net.toLPE.next .t₁ d b .p₁ = d .p₁ \ Bag.single 1 (valueOf b) := by
  rw [CPN.toLPE_next]; exact congrFun (fire_t₁ d b) .p₁

theorem next_t₁_p₂ (d : net.Marking) (b : net.TransBinding .t₁) :
    net.toLPE.next .t₁ d b .p₂ = d .p₂ ∪ Bag.single 1 (valueOf b + 1) := by
  rw [CPN.toLPE_next]; exact congrFun (fire_t₁ d b) .p₂

theorem next_t₁_p₃ (d : net.Marking) (b : net.TransBinding .t₁) :
    net.toLPE.next .t₁ d b .p₃ = d .p₃ := by
  rw [CPN.toLPE_next]; exact congrFun (fire_t₁ d b) .p₃
